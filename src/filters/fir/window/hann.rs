...
mod tests {
    ...
    fn periodicity() {
        const N: usize = 8;
        let config = Config::<f32, N>::new();
        let mut window = Hann::<f32, N>::with_config(config);
    ...
    fn with_config() {
        ...
        let mut window = Hann::<f32, 4>::with_config(config);
    ...
    fn from_guts() {
        ...
        let window: Hann<f32, 4> = FromGuts::from_guts((config, State { k: 2 }));
        let mut w = window;
    ...
    fn into_guts() {
        ...
        let window = Hann::<f32, 4>::with_config(config);
        let (_config, _state) = window.into_guts();
        let restored: Hann<f32, 4> = FromGuts::from_guts((
            Config {
                weights: [1.0f32; 4],
            },
            State::default(),
    ...
    fn zero_window_panics() {
        let _ = Hann::<f32, 0>::with_config(Config {
            weights: [0.0f32; 0],
        });
    }
