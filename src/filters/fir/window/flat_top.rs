...
impl<T: num_traits::Float, const N: usize> Config<T, N> {
    ...
    #[must_use]
    pub fn new() -> Self {
        use crate::filters::fir::convolve::windowed_sinc::flat_top_window;
...
mod tests {
    ...
    fn periodicity() {
        const N: usize = 8;
        let config = Config::<f32, N>::new();
        let mut window = FlatTop::<f32, N>::with_config(config);
    ...
    fn with_config() {
        ...
        let mut window = FlatTop::<f32, 4>::with_config(config);
    ...
    fn from_guts() {
        ...
        let window: FlatTop<f32, 4> = FromGuts::from_guts((config, State { k: 2 }));
        let mut w = window;
    ...
    fn into_guts() {
        ...
        let window = FlatTop::<f32, 4>::with_config(config);
        let (_config, _state) = window.into_guts();
        let restored: FlatTop<f32, 4> = FromGuts::from_guts((
            Config {
                weights: [1.0f32; 4],
            },
            State::default(),
    ...
    fn config_new() {
        ...
        for k in 0..N {
            ...
            let expected = 0.21557895 - 0.41663158 * alpha.cos()
                + 0.277263158 * (2.0 * alpha).cos()
    ...
    #[cfg(any(feature = "libm", feature = "std"))]
    #[test]
    fn flat_top_parity_with_windowed_sinc() {
        use crate::filters::fir::convolve::windowed_sinc::flat_top_window;

        const N: usize = 33;
        let config = Config::<f64, N>::new();
        let win_fn = flat_top_window::<f64>;
    ...
    #[cfg(any(feature = "libm", feature = "std"))]
    #[test]
    fn flat_top_parity_with_windowed_sinc_n2() {
        use crate::filters::fir::convolve::windowed_sinc::flat_top_window;
    ...
    fn zero_window_panics() {
        let _ = FlatTop::<f32, 0>::with_config(Config {
            weights: [0.0f32; 0],
        });
    }
