...
fn smoke() {
    ...
    let mut window = HannArray::<f32, N>::with_config(config.clone());
...
window_behavior_tests!(
    HannArray::<f32, N>::with_config(Config::<[f32; N]>::new()),
    HannArray::<f32, 0>::with_config(Config {
        weights: [0.0f32; 0],
    }),
...
fn from_guts_zero_window_panics() {
    let _: HannArray<f32, 0> = FromGuts::from_guts((
        Config {
            weights: [0.0f32; 0],
        },
        State::default(),
