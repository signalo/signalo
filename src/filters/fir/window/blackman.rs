...
impl<T: num_traits::Float, const N: usize> Config<T, N> {
    ...
    #[must_use]
    pub fn new() -> Self {
        use crate::filters::fir::convolve::windowed_sinc::blackman_window;
...
mod tests {
    ...
    fn smoke() {
        ...
        let mut window = Blackman::<f32, N>::with_config(config.clone());
    ...
    fn periodicity() {
        const N: usize = 8;
        let config = Config::<f32, N>::new();
        let mut window = Blackman::<f32, N>::with_config(config);
    ...
    fn with_config() {
        ...
        let mut window = Blackman::<f32, 4>::with_config(config);
    ...
    fn from_guts() {
        ...
        let window: Blackman<f32, 4> = FromGuts::from_guts((config, State { k: 2 }));
        let mut w = window;
    ...
    fn into_guts() {
        ...
        let window = Blackman::<f32, 4>::with_config(config);
        let (_config, _state) = window.into_guts();
        let restored: Blackman<f32, 4> = FromGuts::from_guts((
            Config {
                weights: [1.0f32; 4],
            },
            State::default(),
    ...
    fn config_new() {
        ...
        for k in 0..N {
            ...
            let expected = 0.42 - 0.5 * alpha.cos() + 0.08 * (2.0 * alpha).cos();
    ...
    fn zero_window_panics() {
        let _ = Blackman::<f32, 0>::with_config(Config {
            weights: [0.0f32; 0],
        });
    }
