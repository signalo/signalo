...
fn smoke() {
    ...
    let mut window = BlackmanHarrisArray::<f32, N>::with_config(config.clone());
...
window_behavior_tests!(
    BlackmanHarrisArray::<f32, N>::with_config(Config::<[f32; N]>::new()),
    BlackmanHarrisArray::<f32, 0>::with_config(Config {
        weights: [0.0f32; 0],
    }),
...
fn blackman_harris_parity_with_windowed_sinc() {
    ...
    const N: usize = 33;
    let config = Config::<[f64; N]>::new();
    let win_fn = blackman_harris::<f64>;
